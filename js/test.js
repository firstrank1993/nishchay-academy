...
async function loadTestIntro(testId) {
  ...
  try {
    ...
    if (!testSnap.exists()) { window.location.href = 'test.html'; return; }
...
window.startTest = async function() {
  ...
  try {
    ...
    userAnswers = {}; questionTimings = {}; reviewFlags = new Set();
    currentQIndex = 0; questionStartTime = new Date(); testStartTime = new Date();
  ...
  nav.innerHTML = allTestQuestions.map((q, i) => `
    <button onclick="goToQuestion(${i})" id="navBtn${i}"
      style="width:34px;height:34px;border-radius:8px;
             ...
             font-family:Inter,sans-serif;">${i+1}</button>
  `).join('');
}
...
function updateNavButton(index) {
  ...
  if (isReview) {
    btn.style.background = '#FEF3C7'; btn.style.borderColor = '#F59E0B'; btn.style.color = '#D97706';
  } else if (answer === -2) {
    btn.style.background = '#E0F2FE'; btn.style.borderColor = '#0284C7'; btn.style.color = '#0284C7';
  } else if (answer !== undefined) {
    btn.style.background = 'var(--success)'; btn.style.borderColor = 'var(--success)'; btn.style.color = 'white';
  } else {
    btn.style.background = 'white'; btn.style.borderColor = 'var(--border)'; btn.style.color = 'var(--text-primary)';
  }
}

window.goToQuestion = function(index) {
  recordQuestionTime();
  currentQIndex = index; questionStartTime = new Date();
  showTestQuestion();
};
...
// ── SHOW QUESTION (with image support) ──
function showTestQuestion() {
  ...
  document.getElementById('reviewBtn').style.opacity    = reviewFlags.has(q.id) ? '1' : '0.4';
  ...
  if (!qImgEl) {
    ...
    qImgEl.id = 'testQuestionImg'; qImgEl.alt = 'Question Image';
    qImgEl.style.cssText = `width:100%;max-height:260px;object-fit:contain;border-radius:10px;margin-top:10px;margin-bottom:4px;border:1px solid var(--border);display:block;`;
    qImgEl.onerror = function() { this.style.display = 'none'; };
    document.getElementById('testQuestionText').insertAdjacentElement('afterend', qImgEl);
  }
  ...
  q.options.forEach((opt, i) => {
    ...
    `;
    const labelHTML = `<span style="width:28px;height:28px;border-radius:50%;background:${isSelected ? 'var(--primary)' : 'var(--primary-light)'};color:${isSelected ? 'white' : 'var(--primary)'};font-weight:700;font-size:13px;flex-shrink:0;display:flex;align-items:center;justify-content:center;">${optLabels[i]}</span>`;
    if (hasOptImage) {
      btn.innerHTML = `${labelHTML}<img src="${q.optionImages[i]}" alt="Option ${optLabels[i]}" style="max-width:calc(100% - 50px);max-height:120px;object-fit:contain;border-radius:6px;pointer-events:none;" />`;
    } else {
      btn.innerHTML = `${labelHTML}<span>${opt}</span>`;
    }
    btn.onclick = () => { userAnswers[q.id] = i; updateNavButton(currentQIndex); showTestQuestion(); };
    container.appendChild(btn);
  });

  const isSkipped = selectedAnswer === -2;
  const skipBtn   = document.createElement('button');
  skipBtn.style.cssText = `width:100%;padding:12px 14px;border-radius:10px;border:${isSkipped ? '2px solid #64748B' : '1.5px solid var(--border)'};background:${isSkipped ? '#F1F5F9' : 'white'};font-size:14px;text-align:left;cursor:pointer;font-family:Inter,sans-serif;transition:all 0.2s;display:flex;align-items:center;gap:10px;`;
  skipBtn.innerHTML = `<span style="width:28px;height:28px;border-radius:50%;background:${isSkipped ? '#64748B' : '#F1F5F9'};color:${isSkipped ? 'white' : '#64748B'};font-weight:700;font-size:13px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">E</span><span style="color:#64748B;">I don't want to attempt this question<span style="font-size:11px;background:#DCFCE7;color:var(--success);padding:2px 6px;border-radius:4px;margin-left:6px;">No negative marks</span></span>`;
  skipBtn.onclick = () => { userAnswers[q.id] = -2; updateNavButton(currentQIndex); showTestQuestion(); };
...
window.prevTestQuestion = function() {
  if (currentQIndex > 0) { recordQuestionTime(); currentQIndex--; questionStartTime = new Date(); showTestQuestion(); }
};

window.nextTestQuestion = function() {
  if (currentQIndex < allTestQuestions.length-1) { recordQuestionTime(); currentQIndex++; questionStartTime = new Date(); showTestQuestion(); }
};
...
window.confirmSubmit = function() {
  ...
  if (unanswered > 0) {
    if (!confirm(`You have ${unanswered} completely unanswered question(s).\n⚠️ Negative marks will apply to these.\n\nSubmit anyway?`)) return;
  } else {
...
function askGuestName() {
  return new Promise((resolve) => {
    ...
    modal.innerHTML = `
      <div style="background:white;border-radius:16px;padding:24px;width:100%;max-width:360px;box-shadow:0 20px 60px rgba(0,0,0,0.3);">
        <div style="font-size:32px;text-align:center;margin-bottom:12px;">👤</div>
        <h3 style="font-size:17px;font-weight:700;text-align:center;margin-bottom:6px;">Enter Your Name</h3>
        <p style="font-size:13px;color:var(--text-secondary);text-align:center;margin-bottom:20px;line-height:1.5;">Your name will be shown on the rank board</p>
        <input id="guestNameInput" type="text" placeholder="Enter your full name" maxlength="40"
          style="width:100%;padding:12px 14px;border:1.5px solid var(--border);border-radius:10px;font-size:15px;font-family:Inter,sans-serif;box-sizing:border-box;margin-bottom:16px;outline:none;" />
        <button id="guestNameSubmit"
          style="width:100%;padding:13px;background:var(--primary);color:white;border:none;border-radius:10px;font-size:15px;font-weight:700;cursor:pointer;font-family:Inter,sans-serif;margin-bottom:10px;">
          Submit Test 🎯
        </button>
        <button id="guestNameCancel"
          style="width:100%;padding:11px;background:transparent;color:var(--text-secondary);border:1.5px solid var(--border);border-radius:10px;font-size:14px;cursor:pointer;font-family:Inter,sans-serif;">
          Cancel
        </button>
      </div>
    `;
    ...
    document.getElementById('guestNameSubmit').onclick = () => {
      const name = document.getElementById('guestNameInput').value.trim();
      if (!name) {
        document.getElementById('guestNameInput').style.borderColor = '#DC2626';
        document.getElementById('guestNameInput').placeholder = 'Please enter your name';
        return;
      }
      modal.remove();
      resolve(name);
    ...
    document.getElementById('guestNameInput').onkeydown = (e) => {
      if (e.key === 'Enter') document.getElementById('guestNameSubmit').click();
    };

    document.getElementById('guestNameCancel').onclick = () => {
      modal.remove();
      resolve(null);
    };
  });
...
async function submitTest() {
  ...
  let totalScore = 0, correct = 0, wrong = 0, skipped = 0, unattempted = 0;
  const answers = {};

  allTestQuestions.forEach(q => {
    const selected = userAnswers[q.id];
    if (selected === undefined) {
      unattempted++; totalScore -= q.negativeMarks || 0;
      answers[q.id] = { selectedOption:-1, isCorrect:false, marksAwarded:-(q.negativeMarks||0), timeTaken:questionTimings[q.id]||0, status:'unattempted' };
    } else if (selected === -2) {
      skipped++;
      answers[q.id] = { selectedOption:-2, isCorrect:false, marksAwarded:0, timeTaken:questionTimings[q.id]||0, status:'skipped' };
    } else if (selected === q.correctOption) {
      correct++; totalScore += q.marksPerQ;
      answers[q.id] = { selectedOption:selected, isCorrect:true, marksAwarded:q.marksPerQ, timeTaken:questionTimings[q.id]||0, status:'correct' };
    } else {
      wrong++; totalScore -= q.negativeMarks || 0;
      answers[q.id] = { selectedOption:selected, isCorrect:false, marksAwarded:-(q.negativeMarks||0), timeTaken:questionTimings[q.id]||0, status:'wrong' };
    }
  });

  const totalTimeTaken = Math.round((new Date() - testStartTime) / 1000);
  const accuracy = (correct + wrong) > 0 ? Math.round((correct / (correct + wrong)) * 100) : 0;

  // Guest mode: ask for name
  let guestName = null;
  if (!currentUser) {
    guestName = await askGuestName();
    if (guestName === null) {
  ...
  try {
    ...
    if (guestName) attemptData.guestName = guestName;
