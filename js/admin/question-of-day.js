    ...
    snap.forEach(d => {
      ...
      card.innerHTML=`
        <div style="margin-bottom:10px;">
          ...
          ${q.explanation?`<p style="font-size:12px;margin-top:6px;padding:8px;background:#f0f9ff;border-radius:8px;">💡 ${q.explanation.substring(0,100)}${q.explanation.length>100?'...':''}</p>`:''}
        </div>
...
window.addQOD = async function() {
  ...
  finally { btn.textContent='Save QOD Entry'; btn.disabled=false; }
};
