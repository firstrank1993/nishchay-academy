...
window.switchTab = function(tab) {
  ...
  ['weekly','monthly','alltime'].forEach(t=>{
    ...
    el.style.cssText=`flex:1;padding:8px 4px;border:none;border-radius:var(--radius-sm);font-size:13px;cursor:pointer;font-family:Inter,sans-serif;${t===tab?'background:var(--primary);color:white;font-weight:700;':'background:transparent;color:var(--text-secondary);font-weight:600;'}`;
  });
...
async function loadLeaderboard(field) {
  ...
  try {
    ...
    const maxStreak=Math.max(...entries.map(e=>e.currentStreak||0));
    document.getElementById('topStreak').textContent=maxStreak;
    const medals=['🥇','🥈','🥉'];
    list.innerHTML='';
    entries.forEach((e,i)=>{
      const card=document.createElement('div');
      card.style.cssText='background:white;border:1.5px solid var(--border);border-radius:var(--radius-md);padding:12px 14px;display:flex;align-items:center;gap:12px;';
      card.innerHTML=`
        <div style="font-size:${i<3?'20':'14'}px;font-weight:800;min-width:32px;text-align:center;color:${i<3?'inherit':'var(--text-secondary)'};">${i<3?medals[i]:`#${i+1}`}</div>
        <div style="flex:1;min-width:0;">
          <p style="font-size:14px;font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${e.fullName||'Student'}</p>
          <p style="font-size:11px;color:var(--text-secondary);margin-top:1px;">${e.district||'Gujarat'} · 🔥 ${e.currentStreak||0} days</p>
        </div>
        <div style="text-align:right;">
          <p style="font-size:15px;font-weight:800;color:var(--primary);">${e[field]||0}</p>
