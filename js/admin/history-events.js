...
onAuthStateChanged(auth, async (user) => {
  ...
  // Auto-set today's day and month display
  const now = new Date();
...
async function loadEvents() {
        ...
        <div style="display:flex;gap:10px;align-items:flex-start;">
          ...
          <div style="flex:1;">
            <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:6px;">
              ...
              ${e.subject?`<span style="font-size:11px;font-weight:700;padding:3px 8px;border-radius:99px;background:#f3e8ff;color:#7c3aed;">${e.subject}</span>`:''}
  ...
  } catch(e) { console.error(e); loader.style.display='none'; showToast('Error loading. Check Firestore indexes.','error'); }
}
...
window.addHistoryEvent = async function() {
  ...
  const subj  = document.getElementById('histSubject').value.trim();
  ...
  try {
    await addDoc(collection(db,'historyEvents'),{day,month,year,title,description:desc,subject:subj||null,imageUrl:image||null,isActive:true,createdAt:serverTimestamp()});
    ['histYear','histTitle','histDesc','histSubject','histImage'].forEach(id=>{document.getElementById(id).value='';});
...
window.downloadHistoryTemplate = function() {
  ...
  const ws = XLSX.utils.aoa_to_sheet([
    ...
    [10,6,1947,'India gained independence','Brief description here','📚 History','']
  ]);
  ...
  try {
    ...
    for (let i=1; i<rows.length; i++) {
      ...
      try {
        ...
        await addDoc(collection(db,'historyEvents'),{day,month,year,title,description:desc,subject:subj||null,imageUrl:img||null,isActive:true,createdAt:serverTimestamp()});
        success++;
...
window.toggleEvent = async function(id,current) {
  try { await updateDoc(doc(db,'historyEvents',id),{isActive:!current}); showToast(current?'Deactivated':'Activated ✅',current?'info':'success'); document.getElementById('histList').style.display='none'; document.getElementById('histLoader').style.display='block'; await loadEvents(); } catch(e) { showToast('Error','error'); }
};
window.deleteEvent = async function(id) {
  if (!confirm('Delete this event?')) return;
  try { await deleteDoc(doc(db,'historyEvents',id)); showToast('Deleted','info'); document.getElementById('histList').style.display='none'; document.getElementById('histLoader').style.display='block'; await loadEvents(); } catch(e) { showToast('Error','error'); }
};
