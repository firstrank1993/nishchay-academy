// ============================================
// NISHCHAY ACADEMY — Admin: Daily Quotes
// ============================================
      ...
      card.innerHTML = `
        <div style="display:flex;gap:12px;align-items:flex-start;">
          ...
          <div style="flex:1;min-width:0;">
            <p style="font-size:14px;font-weight:600;line-height:1.5;margin-bottom:4px;">
              "${q.text}"</p>
            <p style="font-size:12px;color:var(--text-secondary);margin-bottom:8px;">
              — ${q.author}</p>
            <div style="display:flex;flex-wrap:wrap;gap:6px;">
              <span style="font-size:11px;padding:3px 8px;border-radius:99px;font-weight:600;
                background:${isActive ? '#dcfce7' : '#fee2e2'};
                color:${isActive ? '#15803d' : '#dc2626'};">
                ${isActive ? '✅ Active' : '❌ Inactive'}
              </span>
              ${q.scheduledDate ? `<span style="font-size:11px;padding:3px 8px;border-radius:99px;
                background:#e0f2fe;color:#0369a1;font-weight:600;">📅 ${q.scheduledDate}</span>` : ''}
            </div>
          </div>
        </div>
        <div style="display:flex;gap:8px;margin-top:12px;padding-top:12px;
                    border-top:1px solid var(--border);">
          <button onclick="toggleQuote('${q.id}', ${isActive})"
...
window.addQuote = async function() {
  ...
  const btn = document.querySelector('button[onclick="addQuote()"]');
  btn.textContent = 'Saving...'; btn.disabled = true;
  try {
    await addDoc(collection(db, 'dailyQuotes'), {
      text, author, isActive: true,
    ...
    document.getElementById('quotesLoader').style.display = 'block';
    await loadQuotes();
  } catch(e) {
...
window.toggleQuote = async function(id, current) {
  try {
    await updateDoc(doc(db, 'dailyQuotes', id), { isActive: !current });
