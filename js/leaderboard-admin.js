// ============================================
// NISHCHAY ACADEMY — Admin: Leaderboard View
// ============================================
...
const TAB_ON  = 'background:var(--primary);color:white;';
const TAB_OFF = 'background:transparent;color:var(--text-secondary);';

const BASE_STYLE = `flex:1;padding:8px 4px;border:none;border-radius:var(--radius-sm);
...
window.switchTab = function(tab) {
  const tabs = { weekly: 'weeklyPoints', monthly: 'monthlyPoints', alltime: 'totalPoints' };
  ['weekly','monthly','alltime'].forEach(t => {
    const el = document.getElementById(`tab-${t}`);
    el.style.cssText = BASE_STYLE + (t === tab ? TAB_ON + 'font-weight:700;' : TAB_OFF + 'font-weight:600;');
  });
  loadLeaderboard(tabs[tab]);
};
